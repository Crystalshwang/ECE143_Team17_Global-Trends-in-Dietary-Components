...
class Visualization:
    ...
    def generate_map(self,style):
        ...
        for j in range(53):
            ...
            health_oyc=[]
            for i in countries:
                ind1=np.intersect1d((np.where(self.df_crops[:,1]==i))[0],(np.where(self.df_crops[:,3]=='Vegetables'))[0])
    ...
    def generate_pie(self,country):
        ...
        for i in composition1.keys():
            ind1=np.intersect1d((np.where(self.df_crops[:,1]==country))[0],(np.where(self.df_crops[:,3]==i))[0])
            temp=self.df_crops[ind1]
            ind2=(np.where(temp[:,4]==645))[0]
        ...
        for i in composition2.keys():
            ind1=np.intersect1d((np.where(self.df_meats[:,1]==country))[0],(np.where(self.df_meats[:,3]==i))[0])
